...
import com.example.testapplication.databinding.FragmentArithmaticsStatsBinding
...
class ArithmeticsStats : Fragment() {
    ...
    private lateinit var binding: FragmentArithmaticsStatsBinding
    var gamesPlayed = 0
    ...
    override fun onCreateView(
        ...
        binding = FragmentArithmaticsStatsBinding.inflate(inflater,container,false)
        ...
        MyApplication.myRef.child("Users").child(MyApplication.SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child(
            GameNames.ARITHMETICS.toString()).child("GamesPlayed").get().addOnSuccessListener {
            if(it != null){
        ...
        MyApplication.myRef.child("Users").child(MyApplication.SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child(
            GameNames.ARITHMETICS.toString()).child("Win%").get().addOnSuccessListener {
            if(it != null){
