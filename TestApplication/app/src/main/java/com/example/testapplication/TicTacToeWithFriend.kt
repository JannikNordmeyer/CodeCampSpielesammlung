...
class TicTacToeWithFriend : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        MyApplication.myRef.child("Users").child(SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child("Request").get().addOnSuccessListener {
            request = it.value.toString()
        }

        MyApplication.myRef.child("Users").child(SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).addChildEventListener(object : ChildEventListener {
            ...
            override fun onChildChanged(snapshot: DataSnapshot, previousChildName: String?) {
                ...
                if(request != "" && !sendRequest){
                    ...
                    MyApplication.myRef.child("Users").child(SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child("Host").setValue(true)
                    accepted(true)
                }
                stopLoad()
        ...
        createCodeBtn.setOnClickListener {
            ...
            MyApplication.myRef.child("Users").child(SplitString(codeEdt.text.toString())).child("Request").get().addOnSuccessListener {
                ...
                if(busy == "" || busy == null) {
                    MyApplication.myRef.child("Users").child(SplitString(opponent)).child("Request").setValue(FirebaseAuth.getInstance().currentUser!!.email)
                    MyApplication.myRef.child("Users").child(SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child("Request").setValue(opponent)
                    sendRequest = true
                    startLoad()
                //accepted(true)
                } else {
        ...
        joinCodeBtn.setOnClickListener {
            ...
            MyApplication.myRef.child("Users").child(SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child("Host").get().addOnSuccessListener {
                MyApplication.isCodeMaker = it.value as Boolean
                //Toast.makeText(this@TicTacToeWithFriend, isCodeMaker.toString(), Toast.LENGTH_SHORT).show()


                MyApplication.code = if(MyApplication.isCodeMaker) {
                ...
                if(MyApplication.code != "" && MyApplication.code != "null") {
    ...
    fun accepted(host: Boolean) {
        val intent = Intent(this, OnlineMultiplayerGameActivity::class.java)
        MyApplication.code = if(host) {
            SplitString(codeEdt.text.toString()) + SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())
        } else {
            SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString()) + SplitString(request!!)
        }
        Toast.makeText(this@TicTacToeWithFriend, MyApplication.code, Toast.LENGTH_SHORT).show()

        //intent.putExtra("code", code)
        MyApplication.isCodeMaker = host
        //code = "test"
        startActivity(intent)
        stopLoad()
    }
    ...
    fun notification(){
        ...
        build.setPositiveButton("yes"){dialog, which->
            MyApplication.myRef.child("Users").child(SplitString(request!!)).child("Request").setValue(FirebaseAuth.getInstance().currentUser!!.email)
            MyApplication.myRef.child("Users").child(SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child("Host").setValue(false)
            accepted(false)
        }
        build.setNegativeButton("no") {dialog, which->
            MyApplication.myRef.child("Users").child(SplitString(request!!)).child("Request").setValue("")
            MyApplication.myRef.child("Users").child(SplitString(FirebaseAuth.getInstance().currentUser!!.email.toString())).child("Request").setValue("")
            //exitProcess(1)
        }
        build.show()
